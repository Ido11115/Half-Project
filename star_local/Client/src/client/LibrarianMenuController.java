...
/**
 * The LibrarianMenuController class manages the librarian's main menu in the
 * application. It provides functionality to navigate to various
 * librarian-specific screens, such as registering subscribers, loaning books,
...
public class LibrarianMenuController {
	 ...
	 *
	 * @param serverCommunicator the ServerCommunicator used for server
	 *                           communication
	 */
	...
	/**
	 * Handles the logout action, closes the current stage, and returns to the login
	 * screen.
	 *
	...
	private void handleBooks() {
	    try {
	        FXMLLoader loader = new FXMLLoader(getClass().getResource("LibraryBooks.fxml"));
	        Parent root = loader.load();

	        LibraryBooksController controller = loader.getController();
	        ...
	        e.printStackTrace();
	        showError("Error loading Library Books screen: " + e.getMessage());
	    }
	}
